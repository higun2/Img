...
def train_one_epoch(model, model_without_ddp, data_loader, optimizer, device, epoch, log_writer=None, args=None):
    ...
    for data_iter_step, (x, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            diff_loss, add_loss = model(x, labels)

        total_loss = diff_loss + add_loss * 0.5

        total_loss_value = total_loss.item()
        diff_loss_value = diff_loss.item()
        add_loss_value = add_loss.item()
        ...
        metric_logger.update(diff_loss=diff_loss_value)
        metric_logger.update(add_loss=add_loss_value)

        lr = optimizer.param_groups[0]["lr"]
        metric_logger.update(lr=lr)

        total_loss_value_reduce = misc.all_reduce_mean(total_loss_value)
        diff_loss_value_reduce = misc.all_reduce_mean(diff_loss_value)
        add_loss_value_reduce = misc.all_reduce_mean(add_loss_value)

        if log_writer is not None:
            # Use epoch_1000x as the x-axis in TensorBoard to calibrate curves.
            epoch_1000x = int((data_iter_step / len(data_loader) + epoch) * 1000)
            if data_iter_step % args.log_freq == 0:
                log_writer.add_scalar('total_loss', total_loss_value_reduce, epoch_1000x)
                log_writer.add_scalar('diff_loss', diff_loss_value_reduce, epoch_1000x)
                log_writer.add_scalar('add_loss', add_loss_value_reduce, epoch_1000x)
                log_writer.add_scalar('lr', lr, epoch_1000x)

                # Also log to wandb directly for better x-axis control
                # wandb.log({
                #     'total_loss': total_loss_value_reduce,
                #     'diff_loss': diff_loss_value_reduce,
                #     'add_loss': add_loss_value_reduce,
                #     'lr': lr,
...
def evaluate(model_without_ddp, args, epoch, batch_size=64, log_writer=None):
    ...
    if misc.get_rank() == 0 and log_writer is not None:
        ...
        # Clean up memory BEFORE rmtree
        del metrics_dict
        torch.cuda.empty_cache()
        import gc
        gc.collect()

        # Remove folder more safely
        try:
            shutil.rmtree(save_folder)
            print(f"Cleaned up {save_folder}")
        except Exception as e:
            print(f"Warning: Failed to remove {save_folder}: {e}")

    # Barrier after cleanup to ensure all ranks finish
    torch.distributed.barrier()
